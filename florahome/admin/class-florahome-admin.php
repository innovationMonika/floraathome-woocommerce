...
class florahome_Admin {
     ...
     * @since    1.0.0
     */
    function fah_product_options_grouping() {
        exit;
        $args = ['id' => 'text_about_grower', 'label' => sanitize_text_field('About Grower'), 'placeholder' => 'Flora@home about grower ', 'desc_tip' => '', 'description' => '', ];
     ...
     * @since    1.0.0
     */
    public function fah_cron_order_export() {
        ...
        $ordersE = get_posts(array('post_type' => 'shop_order', 'post_status' => $selectedStatus, 'meta_query' => array(array('key' => 'fah_orderExport', 'value' => array('Exported', 'No Flora@home Products', 'No Products for export'), 'compare' => 'NOT IN'))));
        $ordersN = get_posts(array('post_type' => 'shop_order', 'post_status' => $selectedStatus, 'meta_query' => array(array('key' => 'fah_orderExport', 'compare' => 'NOT EXISTS'))));
        $ordersW = array();
        if (count($ordersE) > 0 && count($ordersN) > 0) $ordersW = array_merge($ordersE, $ordersN);
        else if (count($ordersE) > 0) $ordersW = $ordersE;
        else if (count($ordersN) > 0) $ordersW = $ordersN;
        ...
        if (count($ordersW) > 0) {
            foreach ($ordersW as $orderPost) {
                ...
                if ($orderExportStatus != 'Exported' && $orderExportStatus != 'processing') {
                    if ($orderExportStatus) update_post_meta($order->get_id(), 'fah_orderExport', 'processing');
                    else add_post_meta($order->get_id(), 'fah_orderExport', 'processing');
                    try {
                        $fahexport = fah_webshop_order_export($order);
                        if (!empty($fahexport)) {
                            if (isset($fahexport[0])) {
                                if ($fahexport[0]) {
                                    if (get_post_meta($order->get_id(), 'fah_orderExport')) update_post_meta($order->get_id(), 'fah_orderExport', 'Exported');
                                    else add_post_meta($order->get_id(), 'fah_orderExport', 'Exported');
                                } else {
                                    if (isset($fahexport[1]) && !(strpos($fahexport[1], 'No Valid Items in Order') >= 0)) {
                                        $errors[$order->get_id() ] = $fahexport[1];
                                        if (get_post_meta($order->get_id(), 'fah_orderExport')) update_post_meta($order->get_id(), 'fah_orderExport', $fahexport[1]);
                                        else add_post_meta($order->get_id(), 'fah_orderExport', $fahexport[1]);
                                    }
                                    if (strpos($fahexport[1], 'No Valid Items in Order') >= 0) {
                                        delete_post_meta($order->get_id(), 'fah_orderExport');
                                    }
                                }
                            }
                        }
                        ...
                        $orderExportStatus = $order->get_meta('fah_orderExport');
                        if ($orderExportStatus != 'Exported') update_post_meta($order->get_id(), 'fah_orderExport', 'Error: Not exported Try again');
                        $errors[$order->get_id() ] = 'Error while Exporting order: ' . $e->getMessage();
                    }
                }
            }
            if (count($errors) > 0) {
     ...
     * @since    1.0.0
     */
    public function fah_cron_product_update() {
