...
?>
<input type='text' class='flora-field' name='fah_settings[fah_webshop_ref]'
 value='<?php echo esc_attr($options['fah_webshop_ref']??''); ?>' <?php
}
...
function fah_import_schedule_render() {
    ...
    if (!isset($options['fah_select_import_schedule'])) {
        ...
        if (!empty($options['fah_select_import_schedule']) && $options['fah_select_import_schedule'] === 'disable') wp_clear_scheduled_hook('task_flora_product_update');
        else {
            if (wp_get_schedule('task_flora_product_update') !== $options['fah_select_import_schedule']) {
                if ($time = wp_next_scheduled('task_flora_product_update')) wp_unschedule_event($time, 'task_flora_product_update');
...
<select class='flora-field' name='fah_settings[fah_select_import_schedule]'>
 <option value='disable' <?php if (!empty($options['fah_select_import_schedule']) && $options['fah_select_import_schedule'] === 'disable') {
        echo 'selected';
    } ?>>Disable</option>
 <option value='5min' <?php if (!empty($options['fah_select_import_schedule']) && $options['fah_select_import_schedule'] === '5min') {
        echo 'selected';
    } ?>>Every 5 Minutes</option>
 <option value='hourly' <?php if (!empty($options['fah_select_import_schedule']) && $options['fah_select_import_schedule'] === 'hourly') {
        echo 'selected';
    } ?>>every hour</option>
 <option value='twicedaily' <?php if (!empty($options['fah_select_import_schedule']) && $options['fah_select_import_schedule'] === 'twicedaily') {
        echo 'selected';
    } ?>>12 hours</option>
 <option value='daily' <?php if (!empty($options['fah_select_import_schedule']) && $options['fah_select_import_schedule'] === 'daily') {
        echo 'selected';
    } ?>>Once daily</option>
</select>
...
function fah_product_sync_schedule_render() {
    $options = get_option('fah_settings');
    if (!isset($options['fah_select_sync_schedule'])) {
        $options['fah_select_sync_schedule'] = 'disable';
    } else {
        if (!empty($options['fah_select_sync_schedule']) && $options['fah_select_sync_schedule'] === 'disable') wp_clear_scheduled_hook('task_flora_product_sync');
        else {
            if (wp_get_schedule('task_flora_product_sync') !== $options['fah_select_sync_schedule']) {
                if ($time = wp_next_scheduled('task_flora_product_sync')) wp_unschedule_event($time, 'task_flora_product_sync');
                wp_schedule_event(time(), $options['fah_select_sync_schedule'], 'task_flora_product_sync');
            }
        }
    }
    $importlastrun = get_option('fah_select_sync_run');
?>
<select class='flora-field' name='fah_settings[fah_select_sync_schedule]'>
 <option value='disable' <?php if (!empty($options['fah_select_sync_schedule']) && $options['fah_select_sync_schedule'] === 'disable') {
        echo 'selected';
    } ?>>Disable</option>
 <option value='5min' <?php if (!empty($options['fah_select_sync_schedule']) && $options['fah_select_sync_schedule'] === '5min') {
        echo 'selected';
    } ?>>Every 5 Minutes</option>
 <option value='hourly' <?php if (!empty($options['fah_select_sync_schedule']) && $options['fah_select_sync_schedule'] === 'hourly') {
        echo 'selected';
    } ?>>every hour</option>
 <option value='twicedaily' <?php if (!empty($options['fah_select_sync_schedule']) && $options['fah_select_sync_schedule'] === 'twicedaily') {
        echo 'selected';
    } ?>>12 hours</option>
 <option value='daily' <?php if (!empty($options['fah_select_sync_schedule']) && $options['fah_select_sync_schedule'] === 'daily') {
        echo 'selected';
    } ?>>Once daily</option>
</select>
...
function fah_check_publish_render() {
    $options = get_option('fah_settings');
?>
<input type='checkbox' name='fah_settings[fah_check_publish]'
 <?php checked(isset($options['fah_check_publish']) ? $options['fah_check_publish'] : 0, 1); ?> value='1'>
<?php
}
function fah_check_update_attr_render() {
    $options = get_option('fah_settings');
    $checkvalue = 0;
    if (isset($options['fah_check_update_attr'])) $checkvalue = $options['fah_check_update_attr'];
?>
...
<select class='flora-field' name='fah_settings[fah_select_publish_price]' onchange="pricebox(this);">
 <option value='disable' <?php if (!empty($options['fah_select_publish_price']) && $options['fah_select_publish_price'] === 'disable') {
        echo 'selected';
    } ?>>Disable</option>
 <option value='fixed' <?php if (!empty($options['fah_select_publish_price']) && $options['fah_select_publish_price'] === 'fixed') {
        echo 'selected';
    } ?>>Fixed</option>
 <option value='percent' <?php if (!empty($options['fah_select_publish_price']) && $options['fah_select_publish_price'] === 'percent') {
        echo 'selected';
    } ?>>Percentage</option>
...
function fah_text_outofstock_deleted_product_render() {
    $options = get_option('fah_settings');
?>
<input class='flora-field' id="outofstock_deleted" type="checkbox"
 name="fah_settings[fah_text_outofstock_deleted_product_value]"
 <?php checked(isset($options['fah_text_outofstock_deleted_product_value']) ? $options['fah_text_outofstock_deleted_product_value'] : 0, 1); ?>
 value='1'>
<?php
}
function fah_export_schedule_render() {
    ...
    if (!isset($options['fah_select_export_schedule'])) {
        ...
        if ($options['fah_select_export_schedule'] === 'disable') wp_clear_scheduled_hook('task_flora_order_export');
        else {
            if (wp_get_schedule('fah_select_export_schedule') !== $options['fah_select_export_schedule']) {
                if ($time = wp_next_scheduled('task_flora_order_export')) wp_unschedule_event($time, 'task_flora_order_export');
...
<select class='flora-field' name='fah_settings[fah_select_export_schedule]'>
 <option value='disable' <?php if ($options['fah_select_export_schedule'] === 'disable') {
        echo 'selected';
    } ?>>Disable</option>
 <option value='5min' <?php if ($options['fah_select_export_schedule'] === '5min') {
        echo 'selected';
    } ?>>Every 5 Minutes</option>
 <option value='hourly' <?php if ($options['fah_select_export_schedule'] === 'hourly') {
        echo 'selected';
    } ?>>Every hour</option>
 <option value='twicedaily' <?php if ($options['fah_select_export_schedule'] === 'twicedaily') {
        echo 'selected';
    } ?>>12 hours</option>
 <option value='daily' <?php if ($options['fah_select_export_schedule'] === 'daily') {
        echo 'selected';
    } ?>>Once daily</option>
</select>
...
function fah_custom_cron_schedules($schedules) {
    if (!isset($schedules["5min"])) {
        $schedules["5min"] = array('interval' => 5 * 60, 'display' => __('Once every 5 minutes'));
    }
    if (!isset($schedules["5min"])) {
        $schedules["2min"] = array('interval' => 2 * 60, 'display' => __('Once every 2 minutes'));
    }
    if (!isset($schedules["10min"])) {
        $schedules["2min"] = array('interval' => 10 * 60, 'display' => __('Once every 10 minutes'));
    }
    return $schedules;
}
add_filter('cron_schedules', 'fah_custom_cron_schedules');
